...
# ╔═╡ d3af0a40-c8d5-11ec-389b-9ffe496169cb
begin
	using Pluto
	using Plots
...
# ╔═╡ ebb730a3-cad6-419c-9380-1578c001b482
md"""
# Model
"""

# ╔═╡ ca4cdb77-8022-4105-b2b5-e88dbe0216b2
begin
	tis_main = @reaction_network begin 
	#popis pravidel chovani modelu - interakce entit
		  k1*PATO, ∅ --> PATO
		  k2*Er, PATO --> ∅
		  k3*DCa, PATO --> ∅
		  k4*DCi, PATO --> ∅
	...
	end k1 k2 k3 k4 l1 l2 l3 l4 l5 m1 m2

	tis_dead = @reaction_network begin
	#umirani entit s konecnou delkou zivota
		  m3, DCa --> ∅
		  m3, DCi --> ∅
		  d, Er --> ∅
	end m3 d

	@named tis = extend(tis_main, tis_dead)

	ptis = (                #nastaveni parametru
		  :d => 0.001,
		  :k1 => 0.001,
		  :k2 => 0.001,
		  :k3 => 0.001,
	...
	tspantis = (0., 500.)   #nastaveni delky simulace
	utis = [                #nastaveni pocatecnich velikosti populaci entit
		:PATO => 0.001,
		:Er => 0.001,
		:DCa => 0.001,
		:DCi => 0.001
	...
	oprobtis = ODEProblem(tis, utis, tspantis, ptis)
	osoltis = DifferentialEquations.solve(oprobtis, AutoTsit5(Rosenbrock23()))

	Plots.plot(osoltis, ylabel = "Koncentrace entit v tkáni")
end

# ╔═╡ 4b41553f-fabb-4715-9fba-724ea6939ef1
md"""
# Funkce pro vykreslování
"""

# ╔═╡ de52652d-8c5f-49a2-b3fa-2e1df348735a
begin
	#funkce pro vykreslovani prubehu - i s popisy os apod
	function plot_rand_sol(;ps = nothing, vs = nothing, tspantis = (0., 1500.), num = nothing, name = "fig")
		ptis = ps == nothing ? gen_params() : ps
		utis= vs == nothing ? gen_state() : vs
		
		oprobtis = ODEProblem(tis, utis, tspantis, ptis)
		osoltis = DifferentialEquations.solve(oprobtis, AutoTsit5(Rosenbrock23()), dtmax = 1, dtmin = 0.001, force_dtmin = true, maxiters = 10000
		)


		fig = num == nothing ? Plots.plot(osoltis, ylabel = "Relativní množství entit", xlabel = "čas") : (
			num == -1 ?
			Plots.plot(osoltis, legend=true, showaxis = false, foreground_color_text	= :white, ylabel = "Relativní množství entit", xlabel = "čas")
			: Plots.plot(osoltis, legend = true, title = "$num", guidefontsize = 13, tickfontsize = 11, ylabel = "Relativní množství entit", xlabel = "čas"))

		#odkomentovat savefig pro ulozeni obrazku do pocitace
		#savefig(fig, name)
		
		fig, ptis, utis, osoltis
	end
end

# ╔═╡ 1f301174-8c40-4d99-ab5b-ed448469eaf4
odefun = ODEFunction(convert(ODESystem,tis), jac=true) #potreba pro fazovy diagram

# ╔═╡ bb30409d-2326-43d0-a1d5-bda9b21810de
begin
	#funkce pro vykresleni fazoveho diagramu
	function phase_dia(params, fn; minval = 0, maxval = 2, minvaly = 0, maxvaly = 0, steps = 100, plot_num = nothing, title = "title.pdf", label_x = "osa x", label_y = "osa y")
		plot_num = plot_num == nothing ? rand(1:1000000) : plot_num
		fn1(x,y) = fn(params, x, y)
		...
		Y = repeat(range(minvaly,stop=maxvaly,length=steps),1,steps)

		DXY = fn1.(X, Y)
		
		fig = PyPlot.figure("pyplot_streamplot_$plot_num",figsize=(10,10))
		PyPlot.xlabel(label_x, fontsize=16)
		PyPlot.ylabel(label_y, fontsize=16)
		PyPlot.xticks(fontsize=14)
		PyPlot.yticks(fontsize=14)
		PyPlot.subplot()
		PyPlot.streamplot(X,Y,map(first,DXY),map(x->x[2], DXY))
...
# ╔═╡ 49058d21-c3e2-4cf4-b807-50670dfb1757
md"""
# Obrázky
"""

# ╔═╡ 72a45999-ab57-4825-9f7c-2fa87bf66251
plot_rand_sol(
		ps = (
		  :m3 => 0.09758680286138738,
		  :d => 0.05950445984938177,
		  :k1 => 0.03190303489750618,
		  :k2 => 0.04098978889540113,
		  :k3 => 0.041653528031745685,
		  :k4 => 0.09483834978963464,
		  :l1 => 0.07101472752347589,
	      :l2 => 0.06698303427838516,
	      :l3 => 0.058358349138781,
		  :l4 => 0.09724697576046239,
		  :l5 => 0.05432135689025124,
	      :m1 => 1.368785803095296e-5,
	      :m2 => 0.02994314806229509
		),
		...
		tspantis = (0., 1500), 
		name = "vyleceno-fig.pdf"
	)[1]

# ╔═╡ 74a3eb4b-909a-49ea-a1b5-3d6f3e2a7bb1
phase_dia(( 
          #nastaveni parametru
		  0.09758680286138738, 
		  0.05950445984938177,
		  ...
		  0.09724697576046239,
		  0.05432135689025124,
	      1.368785803095296e-5,
	      0.02994314806229509
), (p, x, y) -> Base.invokelatest(odefun, [0.1, 0.1, x, y], p, 0.)[[3,4]]; #v seznamu pocatecnich hodnot entit dame x a y za ty, pro ktere chceme rez fazovym diagramem. Jejich pozice v seznamu pak jeste vyplnime do []
   title = "vyleceno(Er_x_PATO-slice)",
   minval = 0, maxval = 2,
	minvaly = 0, maxvaly = 2,
   steps = 500, label_x = "Er", label_y = "PATO")
...
git-tree-sha1 = "07de1e189209eab9610048b3afe5ff25ea3f88bc"

...
# ╔═╡ a51c0d26-0985-41d2-8ed6-27fa77f8d4ed
begin
	using Pluto
	using Plots
...
# ╔═╡ 546bc8cc-3710-458f-a8df-fa2a1290e05d
begin
	import Random
	Random.seed!(1234)
	nothing
...
# ╔═╡ 86de01c3-57a4-4e85-8002-843c7194205d
md"""
# Model DC
"""

# ╔═╡ 9834e2fd-8642-4045-9e69-432f259b1339
begin
	tis_main = @reaction_network begin 
	#popis pravidel chovani modelu - interakci entit
		  k1*PATO, ∅ --> PATO
		  k2*Er, PATO --> ∅
		  k3*DCa, PATO --> ∅
		  k4*DCi, PATO --> ∅
...
# ╔═╡ efcf89af-e551-4b6a-b31d-19cc56543b50
latexify(convert(ODESystem, tis)) 
#vysazeni diferencialnich rovnic modelu na zaklade pravidel

# ╔═╡ 90ccabba-c52a-4b16-ac17-c6f87476d9fe
md"""
# Náhodné generování
"""

# ╔═╡ 9250ab58-031e-46e4-8b8f-032a1df747a9
begin
	function gen_params() #nahodne generovani parametru z daneho intervalu
	rand.((
		Uniform(0,0.1),        #k1      
		Uniform(0,0.1),        #k2       
		Uniform(0,0.1),        #k3      
	...
	function gen_state() 
	#nahodne generovani pocatecniho mnozstvi entit z daneho intervalu
		rand.( [
		Uniform(0,0.1), #PATO
		Uniform(0,0.1), #Er
		Uniform(0,0.1), #DCa
...
# ╔═╡ 584a2585-090f-4185-aa4d-ccc000944c63
begin
	#funkce pro vykreslovani grafu
	function plot_rand_sol(;ps = nothing, vs = nothing, tspantis = (0., 1500.), num = nothing)
		...
		fig = num == nothing ? Plots.plot(osoltis) : (
			num == -1 ?
			Plots.plot(osoltis, legend=true, showaxis = false, foreground_color_text	= :white)
			: Plots.plot(osoltis, legend = true, title = "$num", guidefontsize = 13, tickfontsize = 11))
		
		fig, ptis, utis, osoltis
	end
end

# ╔═╡ fc9880ed-c9ac-486c-a98c-430e4442b868
begin
	function course_values(osoltis) 
	#funkce pro ukladani dat o casovem prubehu simulace
		values = []
		
		for time in 0:10:1500
			push!(values, osoltis(time))
		end
		Iterators.flatten(values)
	end
end

# ╔═╡ 2f6759b3-e798-4133-89ee-2a20c7066b38
begin
	cnt1 = 0 #nastavit zadouci pocet nahodne inicializovanych simulaci

	local df = DataFrame(
		states_size = [states(tis) |> length],
		params_size = [parameters(tis) |> length],
		states = [states(tis)],
		params = [parameters(tis)]
	)

	CSV.write("params.csv", df, bufsize=2^28) #zapsani poradi parametru a entit
	
	for i in 1:cnt1 #generovani simulaci a obrazku a ukladani obrazku a dat
		x, params, inits, sol = plot_rand_sol(num = -1, tspantis = (0., 1500.))
		course = course_values(sol)
		img = "fig-" * string(i)
		savefig(x, img)

		row = push!(collect(Any, Iterators.flatten([params, course])), img)
		df = DataFrame(
			map(x -> [x], row) |> collect,
			map(x -> string("c_",x) |> Symbol, 1:length(row))	
		)
		print("----\n")

		CSV.write("data.csv", df, append=true, bufsize=2^28) #zapsani dat do vystupniho souboru
	end
end
